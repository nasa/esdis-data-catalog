...
function setupMockResponse(params = null, collectionCount = 20, hits = 2000, prefix = '', appliedFacets = {}) {
  ...
  nock(/cmr/).get(joinUrl(defaultSearchPath, params))
    .reply(200, searchResponse, { 'Cmr-Hits': hits })

  nock(/cmr/).get(joinUrl(defaultFacetsPath, params))
    .reply(200, facetsResponse, { 'Cmr-Hits': hits })
}
...
describe('DataCatalog', () => {
  ...
  test('request new facet after checking a facet', async () => {
    ...
    expect(await screen.findByText('Found collection 1')).toBeTruthy()
    expect(screen.getByLabelText('Found Keyw0 (10)').checked).toEqual(false)
    expect(screen.getByLabelText('Found Keyw1 (20)').checked).toEqual(true)
  ...
  describe('When filters have been applied', () => {
    ...
    const doClickFilterTest = async (filterName, removedParamName) => {
      const filterParams = [
    ...
    test('clicking to remove a temporal filter removes the temporal filter', async () => {
      ...
      expect(screen.getByLabelText('Found Keyw1 (20)').checked).toEqual(true)
      expect(screen.getByLabelText('Found Plat1 (20)').checked).toEqual(true)
    })
  ...
  describe('responsive filters sidebar', () => {
    ...
    beforeEach(() => useMediaQuery.mockImplementation(() => false))
    ...
    test('clicking the close button on the sidebar closes the sidebar', async () => {
      ...
      await waitFor(async () => {
        await user.click(within(filterHeader).getByLabelText('Close'))
      })
  ...
  test('clicking the "Apply" button on the sidebar closes the sidebar', async () => {
    ...
    await waitFor(() => user.click(within(filterHeader).getByRole('button', { name: 'Apply' })))
  ...
  describe('when clicking a filter group', () => {
    ...
    test('displays a close button that closes all sidebars', async () => {
      ...
      expect(within(sectionHeader).getByLabelText('Close')).toBeInTheDocument()
      await waitFor(() => user.click(within(sectionHeader).getByLabelText('Close')))
    ...
    test('displays an "Apply" button that closes all sidebars', async () => {
      ...
      await waitFor(async () => {
        ...
        await user.click(within(sectionHeader).getByRole('button', { name: 'Apply' }))
      })
